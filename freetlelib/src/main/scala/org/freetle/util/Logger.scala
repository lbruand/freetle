...
*/
  def apply(clazz: Class[_], locationInfo : String): Logger = {
    require(clazz != null, "clazz must not be null!")
    logger(LoggerFactory getLogger clazz, locationInfo)
  }
...
*/
  def apply(name: String, locationInfo : String): Logger = {

    require(name != null, "loggerName must not be null!")
    logger(LoggerFactory getLogger name, locationInfo)
  }

  private def logger(slf4jLogger: Slf4jLogger, locationInfo : String): Logger = slf4jLogger match {
    case locationAwareLogger: Slf4jLocationAwareLogger =>
      new DefaultLocationAwareLogger(locationAwareLogger, locationInfo)
    case _ =>
...
*/
  ...
  protected val locationInfo : String
...
private[util] final class DefaultLocationAwareLogger(
                              override protected val slf4jLogger: Slf4jLocationAwareLogger,
                              override protected val locationInfo : String)
